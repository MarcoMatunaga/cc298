...
use vars 
implicit none
real(8),dimension(:,:),allocatable           :: p_out, u_out, v_out
!
!
!
allocate(p_out(imax,jmax), u_out(imax,jmax), v_out(imax,jmax) )
!
...
do j = 1, jmax
    do i = 1, imax
        !write(3,'(7es11.3e2)') meshx(i,j), meshy(i,j), x_ksi(i,j), x_eta(i,j), y_ksi(i,j), y_eta(i,j), metric_jacobian(i,j)
        !write(3,*) meshx(i,j), meshy(i,j), x_ksi(i,j), x_eta(i,j), y_ksi(i,j), y_eta(i,j), metric_jacobian(i,j)
        write(3,'(6EN20.10)') meshx(i,j), meshy(i,j), u_out(i,j), v_out(i,j), Q(i,j,1), p_out(i,j) 
    end do
end do
close(3)
deallocate(u_out, v_out, p_out)
!
!
!
