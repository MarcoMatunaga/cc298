subroutine  blktriad_pc(maind,lower,upper,id,md,xb,x)
    ...
    do i = 2, md
        ...
        do ii = 1, id
            aux_summ(ii,ii) = 1.0d0/aux_summ(ii,ii)
            aux_dumm(ii) = xb(ii,i)-lower(ii,ii,i)*beta(ii,i-1)
            beta(ii,i) = aux_summ(ii,ii)*aux_dumm(ii)
        end do 
    ...
    do i = md-1,1,-1
        ...
        do ii = 1, id
            aux_dumm(ii) = gamm(ii,ii,i)*x(ii,i+1)
